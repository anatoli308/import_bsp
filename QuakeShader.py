...
class vanilla_shader_stage:
    def __init__(stage):
        ...
        stage.stage_functions = {
            "map": stage.setDiffuse,
...
class quake_shader:
    ...
    def get_alphaGen_node(shader, alphaGen, alpha_value):
        if alphaGen == ALPHA_UNDEFINED:
            ...
            if shader.is_grid_lit:
                shader.links.new(
                    shader.get_rgbGen_node(LIGHTING_LIGHTGRID).outputs[
                        "LightGridVector"
                    ],
                ...
                shader.links.new(
                    shader.get_node_by_name("BaseReflectionVector").outputs["Vector"],
                    spec_node.inputs["LightVector"],
                )
            return spec_node
        else:
        ...
        for tcMod, arguments in zip(tcMods, tcMod_arguments):
            if tcMod == "scale":
                ...
                if out_node is not None:
                    shader.links.new(out_node.outputs[0], new_out_node.inputs["Vector"])
                ...
                new_out_node.name = "tcMod"
                new_out_node.node_tree = ShaderNodes.Shader_Rotate_Node.get_node_tree(
                    None
                )
                ...
                new_out_node = shader.nodes.new(type="ShaderNodeGroup")
                new_out_node.name = "tcMod"
                new_out_node.node_tree = ShaderNodes.Shader_Scroll_Node.get_node_tree(
                    None
                )
                new_out_node.location = (
                    shader.current_x_location - 200,
        ...
        if stage.valid:
            ...
            if img is not None:
                ...
                if tc_gen is not None:
                    ...
                    if tc_mod is None:
                        shader.links.new(
                            tc_gen.outputs["UV"], node_color.inputs["Vector"]
                        )
                    else:
                        shader.links.new(tc_gen.outputs["UV"], tc_mod.inputs["Vector"])
                        shader.links.new(tc_mod.outputs[0], node_color.inputs["Vector"])
                ...
                if stage.blend != BLEND_NONE:
                    ...
                    node_blend.node_tree = ShaderNodes.Blend_Node.get_node_tree(
                        stage.blend
                    )
                    ...
                    if alpha_node is None:
                        ...
                        shader.links.new(
                            alpha_node.outputs[0], node_blend.inputs["SourceAlpha"]
                        )
    ...
    def finish_rendering_shader(shader, VFS, import_settings):
        ...
        if (
            shader.is_system_shader
            or "skyparms" in shader_atts
            or "fogparms" in shader_atts
        ):
            shader.nodes.clear()
            node_output = shader.nodes.new(type="ShaderNodeOutputMaterial")
            node_output.name = "Output"
            ...
            if "skyparms" in shader_atts:
                ...
                if image is None:
                    image = QuakeSky.make_equirectangular_from_sky(VFS, skyname)
                ...
                if lt_node is None:
                    ...
                    lt_node.location = (
                        im_node.location[0] + 200,
                        im_node.location[1] + 400,
                    )
                ...
                if mx_node is None:
                    ...
                    mx_node.location = (
                        im_node.location[0] + 600,
                        im_node.location[1] + 300,
                    )
            ...
            for stage in shader.stages:
                if stage_index == 0:
                    ...
                    if (
                        stage.blend.endswith("gl_one_minus_src_alpha")
                        and shader_type == "ADD"
                    ):
                        shader_type = "BLEND"
                        shader.mat.blend_method = "BLEND"
                    if stage.blend.endswith("gl_src_alpha") and shader_type == "ADD":
                        ...
                        shader.mat.blend_method = "BLEND"
                    if (
                        stage.blend.endswith("gl_src_color")
                        and shader_type == "ADD"
                        and not stage.lightmap
                    ):
                        shader_type = "MULTIPLY"
                        shader.mat.blend_method = "BLEND"
                    if (
                        stage.blend.startswith("gl_dst_color")
                        and shader_type == "ADD"
                        and not stage.lightmap
                    ):
                        shader_type = "MULTIPLY"
                ...
                if stage.blend.endswith("gl_zero") and not stage.skip_alpha:
                    ...
                    shader.mat.blend_method = (
                        "OPAQUE" if shader.mat.blend_method != "CLIP" else "CLIP"
                    )
                ...
                if (
                    stage.lighting == LIGHTING_VERTEX
                    or stage.lighting == LIGHTING_LIGHTGRID
                ):
                    stage.lighting = 0
                ...
                if stage.glow or stage.blend == "gl_one gl_one":
                    out_Glow, out_None = shader.build_stage_nodes(
                        VFS, stage, out_Glow, out_Alpha
                    )
                else:
                    out_Color, out_Alpha = shader.build_stage_nodes(
                        VFS, stage, out_Color, out_Alpha
                    )
            ...
            if (
                "q3map_normalimage" in shader.attributes
                and import_settings.normal_map_option != NormalMapOption.SKIP.value
            ):
                ...
                if normal_img is not None:
                    ...
                    node_channelflip.mute = (
                        import_settings.normal_map_option
                        != NormalMapOption.DIRECTX.value
                    )
            ...
            if out_Color is not None:
                ...
                max_node.location = (node_BSDF.location[0] - 200, node_BSDF.location[1])
                ...
                if out_Glow is not None or node_light is not None:
                    ...
                    if "q3map_surfacelight" in shader.attributes:
                        try:
                            ...
                            print(
                                "Could not set q3map_surfacelight to",
                ...
                if (
                    shader_type != "OPAQUE"
                    and out_Alpha is not None
                    and "portal" not in shader.attributes
                ):
        ...
        if lm_image is None:
            lm_image = bpy.data.images.new(
                "$lightmap_bake", width=lm_size, height=lm_size, float_buffer=True
            )
        vt_image = bpy.data.images.get("$vertmap_bake")
        if vt_image is None:
            vt_image = bpy.data.images.new(
                "$vertmap_bake", width=2048, height=2048, float_buffer=True
            )
        ...
        if "cull" in shader.attributes:
            if (
                shader.attributes["cull"][0] == "twosided"
                or shader.attributes["cull"][0] == "none"
                or shader.attributes["cull"][0] == "disable"
            ):
    ...
    def finish_preview_shader(shader, VFS, import_settings):
        ...
        if shader.is_system_shader or "fogparms" in shader.attributes:
        ...
        if "skyparms" in shader.attributes:
            ...
            for stage in shader.stages:
                if stage_index == 0:
                    ...
                    if stage.blend != BLEND_NONE:
                        shader_type = "ADD"
                        shader.mat.blend_method = "BLEND"
                    if shader.is_vertex_lit and stage.lighting is LIGHTING_NONE:
                        stage.lighting = LIGHTING_VERTEX
                    if shader.is_grid_lit and stage.lighting is LIGHTING_NONE:
                        stage.lighting = LIGHTING_LIGHTGRID
                ...
                if stage.depthwrite:
                    ...
                    if (
                        stage.alpha_clip != ACLIP_NONE
                        and shader.mat.blend_method != "OPAQUE"
                    ):
                        shader_type = "CLIP"
                        shader.mat.blend_method = "CLIP"
                ...
                if shader.is_vertex_lit and stage.lightmap:
                    ...
                    if color_out is None:
                        color_out = shader.get_rgbGen_node(LIGHTING_LIGHTGRID).outputs[
                            0
                        ]
                    elif shader.last_blend is not None:
                        shader.links.new(
                            shader.get_rgbGen_node(LIGHTING_LIGHTGRID).outputs[0],
                            shader.last_blend.inputs["rgbGen"],
                        )
                else:
                    if stage.skip_alpha:
                        color_out, none_out = shader.build_stage_nodes(
                            VFS, stage, color_out, alpha_out
                        )
                    else:
                        color_out, alpha_out = shader.build_stage_nodes(
                            VFS, stage, color_out, alpha_out
                        )
            ...
            if img is not None:
                ...
                if shader.is_vertex_lit:
                    ...
                    shader.links.new(
                        node_texture.outputs["Color"], node_blend.inputs["Color1"]
                    )
                    shader.links.new(
                        shader.get_rgbGen_node(LIGHTING_VERTEX).outputs[0],
    ...
    def finish_brush_shader(shader, VFS, import_settings):
        ...
        if "qer_editorimage" in shader.attributes:
            image = BlenderImage.load_file(shader.attributes["qer_editorimage"][0], VFS)
        else:
        ...
        if (
            ...
            or import_settings.preset == "SHADOW_BRUSHES"
        ):
            transparent = False
    ...
    def finish_fog_shader(shader, VFS, import_settings):
        ...
        if import_settings.preset != "RENDERING":
            ...
            shader.links.new(
                node_density.outputs[0], node_Voulme.inputs["Emission Strength"]
            )

        node_Voulme.name = "Out_Volume"
        node_Voulme.location = (3000, 0)
        shader.links.new(node_Voulme.outputs["Volume"], node_output.inputs[1])
    ...
    def finish_shader(shader, VFS, import_settings):
        if shader.is_fog:
            ...
            import_settings.preset != "RENDERING"
            and import_settings.preset != "BRUSHES"
        ):
...
def get_shader_image_sizes(VFS, import_settings, material_list):
    return ID3Shader.get_shader_image_sizes(VFS, import_settings, material_list)
...
def build_quake_shaders(VFS, import_settings, object_list):
    ...
    for shader in shaders:
        ...
        if "surfaceparm" in attributes and "nodraw" in attributes["surfaceparm"]:
        ...
        for current_shader in current_shaders:
            ...
            if "cull" in attributes:
                current_shader.mat["two_sided"] = (
                    attributes["cull"][0] == "disable"
                    or attributes["cull"][0] == "none"
                    or attributes["cull"][0] == "twosided"
                )
            if "skyparms" in attributes:
                current_shader.mat["is_sky"] = True
            if "fogparms" in attributes:
                current_shader.mat["is_fog"] = True
        ...
        for current_shader in current_shaders:
            ...
            for shader_stage in current_shader.stages:
                if shader_stage.tcGen == TCGEN_LM and shader_stage.diffuse.startswith(
                    "maps/"
                ):
...
def prepare_textures_for_unity(VFS, import_settings, object_list, output_dir=None):
    import os
    import time

    print("Starting prepare_textures_for_unity optimization...")
    start_time = time.time()

    # OPTIMIZATION: Collect unique material base names more efficiently
    # Use a set for O(1) lookup instead of list with O(n) lookup
    material_names_set = set()
    ...
    for i in range(0, len(object_list), batch_size):
        ...
        for obj in batch:
            if obj.data is None or obj.data.name == "box":
                continue
            for m in obj.material_slots:
                ...
                idx = name.find(".")
                shader_name = name.split(".")[0] if idx != -1 else name
                material_names_set.add(shader_name)
        
        processed_objects += len(batch)
        if processed_objects % 2000 == 0:
            elapsed = time.time() - start_time
            print("Processed {} objects in {:.2f}s".format(processed_objects, elapsed))
    
    # Convert set back to list for compatibility
    material_names = list(material_names_set)
    
    elapsed = time.time() - start_time
    print("Material collection completed: {} unique materials from {} objects in {:.2f}s".format(
        len(material_names), processed_objects, elapsed))

    if len(material_names) == 0:
        return

    # Query shader info to find referenced textures
    print("Querying shader info for {} materials...".format(len(material_names)))
    shader_query_start = time.time()
    
    shader_info = ID3Shader.get_material_dicts(
        VFS, import_settings, [l_format(n) for n in material_names]
    )
    
    shader_query_elapsed = time.time() - shader_query_start
    print("Shader query completed in {:.2f}s".format(shader_query_elapsed))

    # OPTIMIZATION: Cache texture path resolution to avoid repeated file system calls
    texture_path_cache = {}
    
    # Resolve a texture path from VFS.basepaths without copying/exporting
    def resolve_texture_path(rel_without_ext):
        # Check cache first
        if rel_without_ext in texture_path_cache:
            return texture_path_cache[rel_without_ext]
        
        rel_norm = rel_without_ext.replace("\\", "/").lstrip("/")
        extensions = [".tga", ".png", ".dds", ".jpg", ".jpeg"]
        for base in getattr(VFS, "basepaths", []) or []:
            for ext in extensions:
                candidate = os.path.join(base, rel_norm + ext)
                if os.path.exists(candidate):
                    texture_path_cache[rel_without_ext] = candidate
                    return candidate
        # also try DDS/ subfolder as used sometimes
        for base in getattr(VFS, "basepaths", []) or []:
            for ext in [".dds"]:
                candidate = os.path.join(base, "DDS", rel_norm + ext)
                if os.path.exists(candidate):
                    texture_path_cache[rel_without_ext] = candidate
                    return candidate
        
        # Cache negative result too
        texture_path_cache[rel_without_ext] = None
        return None

    def assign_image_to_filesystem(img, rel_name):
        if img is None:
            return
        # Strip extension to search
        rel_no_ext = rel_name
        for ext in (
            ".tga",
            ".png",
            ...
            if rel_no_ext.lower().endswith(ext):
                rel_no_ext = rel_no_ext[: -len(ext)]
                break
        fs_path = resolve_texture_path(rel_no_ext)
        if fs_path is None:
            return
        try:
            if getattr(img, "packed_file", None) is not None:
                try:
                    ...
                    pass
            img.filepath = fs_path
            img.reload()
        except Exception as e:
            print("Failed to assign path to image:", img.name, fs_path, e)

    # Export textures referenced by shaders
    for shader_key in shader_info:
        attributes, stages = shader_info[shader_key]

        # Stage textures
        for stage in stages:
            tex = None
            if "map" in stage:
                tex = stage["map"].split()[0]
            elif "clampmap" in stage:
                tex = stage["clampmap"].split()[0]
            elif "animmap" in stage:
                parts = stage["animmap"].split()
                if len(parts) >= 2:
                    tex = parts[1]
            if tex is None:
                continue
            ...
            if img is None:
                img = BlenderImage.load_file(tex, VFS)
            if img is not None:
                assign_image_to_filesystem(img, tex)

        # Attribute-driven textures
        if "q3map_normalimage" in attributes:
            tex = attributes["q3map_normalimage"][0]
            img = bpy.data.images.get(tex)
            if img is None:
                img = BlenderImage.load_file(tex, VFS)
            if img is not None:
                ...
                assign_image_to_filesystem(img, tex)
        if "q3map_lightimage" in attributes:
            tex = attributes["q3map_lightimage"][0]
            img = bpy.data.images.get(tex)
            if img is None:
                img = BlenderImage.load_file(tex, VFS)
            if img is not None:
    ...
    # Optionally remap generated sky textures too
    for img in bpy.data.images:
        if img.name.startswith("env/") or img.name.startswith("textures/skies/"):
            assign_image_to_filesystem(img, img.name)

    # Rewire Blender materials to simple Principled setup so FBX exports texture slots Unity can pick up
    # Build a quick lookup
    def pick_albedo_from_stages(stages_list):
        for st in stages_list:
            tex = None
            if "map" in st:
                tex = st["map"].split()[0]
            elif "clampmap" in st:
                tex = st["clampmap"].split()[0]
            elif "animmap" in st:
                parts = st["animmap"].split()
                if len(parts) >= 2:
                    tex = parts[1]
            if tex is None:
                continue
            if tex == "$lightmap":
                continue
            tc = st.get("tcgen", "")
            if isinstance(tc, str) and tc.startswith("environment"):
                continue
            return tex
        return None

    # Map of base shader name to (attributes, stages)
    base_to_info = {}
    for key, value in shader_info.items():
        base_to_info[key] = value

    # OPTIMIZATION: Process materials in batches and cache processed materials
    print("Starting material processing optimization...")
    material_processing_start = time.time()
    
    # Cache for already processed materials to avoid duplicate work
    ...
    for obj in object_list:
        ...
        for mslot in obj.material_slots:
            ...
            name = mat.name
            idx = name.find(".")
            shader_base = name.split(".")[0] if idx != -1 else name
            info = base_to_info.get(l_format(shader_base))
            if info is None:
                continue
            attributes, stages = info
            # Choose textures
            albedo_tex = pick_albedo_from_stages(stages)
            normal_tex = (
                attributes.get("q3map_normalimage", [None])[0]
                if "q3map_normalimage" in attributes
                else None
            )
            ...
            if albedo_tex is not None:
                albedo_img = bpy.data.images.get(albedo_tex) or BlenderImage.load_file(
                    albedo_tex, VFS
                )
            if normal_tex is not None:
                normal_img = bpy.data.images.get(normal_tex) or BlenderImage.load_file(
                    normal_tex, VFS
                )
                if normal_img is not None:
                    normal_img.colorspace_settings.name = "Non-Color"

            # Rebuild minimal node tree only if we have at least albedo
            ...
            # Transparency guess: if albedo has alpha channel, wire it to Alpha
            try:
                has_alpha = albedo_img.channels >= 4
            except Exception:
                has_alpha = True
            if has_alpha:
            ...
            if normal_img is not None:
                ...
                ntex.location = (-200, -300)
                ntex.image = normal_img
                ntex.image.colorspace_settings.name = "Non-Color"
                nrm = nodes.new(type="ShaderNodeNormalMap")
                nrm.location = (0, -300)
                nrm.uv_map = "UVMap"
                links.new(ntex.outputs.get("Color"), nrm.inputs.get("Color"))
                links.new(nrm.outputs.get("Normal"), bsdf.inputs.get("Normal"))

            # Link BSDF to Output Surface using named sockets
            try:
                links.new(bsdf.outputs["BSDF"], out.inputs["Surface"])
            except Exception:
                # Fallback to first input if names differ in version
                links.new(bsdf.outputs[0], out.inputs[0])
        
        processed_objects_count += 1
        if processed_objects_count % 1000 == 0:
            elapsed = time.time() - material_processing_start
            print("Processed {} objects, {} unique materials in {:.2f}s".format(
                processed_objects_count, len(processed_materials), elapsed))

    total_elapsed = time.time() - start_time
    print("prepare_textures_for_unity completed: {} objects, {} unique materials in {:.2f}s".format(
        processed_objects_count, len(processed_materials), total_elapsed))
